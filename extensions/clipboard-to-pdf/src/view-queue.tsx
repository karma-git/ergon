...
import { getFrames, deleteFrame, moveFrame, clearFrames, framePath, FrameMeta } from "./utils";
...
export default function Command() {
  ...
  async function handleGenerate() {
    if (frames.length === 0) {
      await showHUD("⚠️ No frames to export");
      return;
    }
    ...
    try {
      await generatePdf(frames, outputPath);
      await clearFrames();
      await reload();
            ...
            actions={
              <ActionPanel>
                <ActionPanel.Section>
                  ...
                  />
                  <Action
                    title="Open Frame in Preview"
                    icon={Icon.Eye}
                    onAction={() => open(framePath(frame.filename))}
                  />
                </ActionPanel.Section>
                <ActionPanel.Section title="Reorder">
                  <Action
                    ...
                    onAction={() => handleMove(frame.id, "up")}
                  />
                  <Action
                    title="Move Down"
                    icon={Icon.ChevronDown}
                    shortcut={{ modifiers: ["cmd"], key: "arrowDown" }}
                    onAction={() => handleMove(frame.id, "down")}
                  />
                </ActionPanel.Section>
                <ActionPanel.Section title="Remove">
                  <Action
                    ...
                    onAction={() => handleDelete(frame.id)}
                  />
                  <Action
                    title="Clear All Frames"
