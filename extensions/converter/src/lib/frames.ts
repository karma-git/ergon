...
export const framesDir = () => FRAMES_DIR;
export const framePath = (filename: string) => path.join(FRAMES_DIR, filename);
...
export async function deleteFrame(id: string): Promise<void> {
  ...
  if (frame) { const p = framePath(frame.filename); if (fs.existsSync(p)) fs.unlinkSync(p); }
...
export async function clearFrames(): Promise<void> {
  const frames = await getFrames();
  for (const f of frames) { const p = framePath(f.filename); if (fs.existsSync(p)) fs.unlinkSync(p); }
  await saveFrames([]);
}
